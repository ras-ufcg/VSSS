...
Utils::Posture blueObjective(vss::State state, int index, vss::Robot goleiro, vss::Robot outro)
{
	...
	if(estado == 'a')
	{
		...
		else if((((bolinha.xb - xd) < d3) && (y_min <= (bolinha.yb + (((yd - bolinha.yb) * (bolinha.xb - xg)) / (xd - bolinha.xb)))) && (y_max >= (bolinha.yb + (((yd - bolinha.yb) * (bolinha.xb - xg)) / (xd - bolinha.xb))))) && area_t(bolinha.xb, bolinha.yb, xd, yd)){	
			estado = 'd';
			xa = robot.x;
			ya = robot.y;	
			xd = outro.x;
	...
	{
		// Regra 4 (permuta de posições)
		if((((bolinha.xb - xd) < d3) && (y_min <= (bolinha.yb + (((yd - bolinha.yb) * (bolinha.xb - xg)) / (xd - bolinha.xb)))) && (y_max >= (bolinha.yb + (((yd - bolinha.yb) * (bolinha.xb - xg)) / (xd - bolinha.xb))))) && area_t(bolinha.xb, bolinha.yb, xd, yd)){
			estado = 'b'; 
			xd = robot.x;
			yd = robot.y;	
			xa = outro.x;
		...
		// Regra 5
		else if(((bolinha.xb - xa) < d3) && (y_min <= (bolinha.yb + (((ya - bolinha.yb) * (bolinha.xb - xg)) / (xa - bolinha.xb)))) && (y_max <= (bolinha.yb + (((ya - bolinha.yb) * (bolinha.xb - xg)) / (xa - bolinha.xb)))))
			estado = 'd';
		else 
			estado = 'e';
	}
